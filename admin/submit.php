...
if (isset ($_POST['submit'])) {
	...
	if (sizeof ($Errors) == 0) {
		$SQL = 'UPDATE problems set category_id=%d, status_id=%d, lat=%f, lon=%f, `timestamp`=%d, address=%s, description=%s, reply=%s, name=%s, email=%s, ip=%d where case_id=%d';
		
		$SQL = sprintf ($SQL, $_POST['categories'], $_POST['status'], $_POST['lat'], $_POST['lon'], time(), quote_smart ($_POST['address']), quote_smart ($_POST['description']), quote_smart ($_POST['reply']), quote_smart ($_POST['name']), quote_smart ($_POST['email']), ip2long ($_SERVER['REMOTE_ADDR']), quote_smart ($_GET['case_id']));
		mysql_query ($SQL);
